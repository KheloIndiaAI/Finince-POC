...
6. mismatch_note — use this ONLY when the two documents are about different
   things, so the comparison itself would be meaningless. It is a strong
   claim; the default answer is "". Report a mismatch only if one of these
   is true:
     - different state / centre / implementing body (e.g. the sanction is for
       Uttarakhand, the UC is from the Govt. of Tripura),
     - different financial year (e.g. sanction for FY 2024-25, UC certifies
       FY 2023-24),
     - the UC is certifying a clearly different sanction: a different
       sanctioned amount AND a different purpose from the Sanction Order.
   Quote the words from each document that show it. Report only what the
   documents actually say — never guess a state or a year that is not
   written. If none of the three is true, return "".

   NOT a mismatch, and must never go in this field:
     - the UC citing a sanction letter number, file number or reference in a
       different format from the Sanction Order's own file number,
     - dates differing by a few days (an administrative sanction letter is
       routinely issued a day or two after the sanction order),
     - the UC covering more quarters, or a longer period, than the sanction,
     - the UC reporting a higher or lower amount than was sanctioned,
     - an earlier or superseded UC appearing in the same document.
   These are ordinary paperwork differences. They belong in
   consistency_note, and the pair is still a matching pair.

7. consistency_note — one short sentence on differences worth a reviewer's
   eye that do NOT mean the documents are unrelated: the reference/file
   numbers or dates not lining up exactly, the UC covering a different or
   longer period than the sanction (e.g. sanction covers Q1-Q3, the UC
   certifies Q1-Q4), an earlier superseded UC with a different figure, or
   anything else that a reviewer should reconcile. Quote the relevant words.
   Return "" if there is nothing to note.
...
KEEP EVERY TEXT FIELD SHORT. A reviewer reads these next to the document, so
they only need the decisive words, not the surrounding paragraph:
  - every *_snippet: at most 200 characters. Quote the single line or clause
    that carries the figure or the purpose. Do not include the letterhead,
    the file number, or the sentences around it.
  - mismatch_note and consistency_note: one sentence, at most 240 characters.
    Name the difference and quote only the few words that prove it. If there
    is more than one point, give the most important one and stop.
Never shorten or round a NUMBER to save space - amounts are exact.
...
Return exactly this JSON shape and nothing else:
{
  ...
  "consistency_note": "<text or empty string>"
}
